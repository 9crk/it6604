...
VideoTimerHandler()
{
    ...
    if(VState == VSTATE_SyncWait)
    {
        ...
        if(VideoCountingTimer == 0)
        {
            HDMIRX_DEBUG_PRINTF(("VsyncWaitResetTimer up, call SWReset_HDMIRX() %d\n",VideoCountingTimer));
            SWReset_HDMIRX();
            return ;
        ...
        else
        {
            ...
            //
            // 	if(uc ==(B_RXCK_VALID|B_VCLK_DET))// for check DVI mode and time out count
            // 	{
            // 		if(ucDVISCDToffCNT++>100)
            // 		{
            // 				HDMIRX_WriteI2C_Byte(REG_RX_RST_CTRL, B_SWRST|B_EN_AUTOVDORST);
    ...
    if(VState == VSTATE_VideoOn)
    {
		...
		if(!bVidModeChange)
		{
			...
			diff = (int)currXcnt - (int)xCnt ;
#if 0
      if(diff!=0){
					HDMIRX_DEBUG_PRINTF(("Xcnt changed. %02x -> %02x, diff=%d\r\n ",(int)xCnt,(int)currXcnt, (int)diff));
      }
#endif      
			if(xCnt > 0x80)
			{
        ...
        if(VideoCountingTimer == 1)
        {
            ...
            if(ucRevision >= 0xA3)
            {
                ...
                {
                    if((HDMIRX_ReadI2C_Byte(REG_RX_GENPKT_HB0)&0xE0)== 0x40)
                    {
                        diff = HDMIRX_ReadI2C_Byte(0x3C);
                        diff |=(1<<2);
                        HDMIRX_WriteI2C_Byte(0x3C,diff);
                    }
                }
...
//    pVTiming = &s_CurrentVM ;
	...
	if((s_CurrentVM.VActive > 200)&&(s_CurrentVM.VTotal>s_CurrentVM.VActive)&&
(s_CurrentVM.HActive > 300)&&(s_CurrentVM.HTotal>s_CurrentVM.HActive))
	{
		return TRUE ;
	}
#else
...
Video_Handler()
{
    ...
    if(VState == VSTATE_ModeDetecting)
    {
        ...
        if(!bGetSyncInfo())
        {
            HDMIRX_DEBUG_PRINTF2(("Current Get: ")); DumpSyncInfo(&s_CurrentVM);

            SwitchVideoState(VSTATE_SyncWait);
            bGetSyncFailCount ++ ;
            HDMIRX_DEBUG_PRINTF(("bGetSyncInfo()fail, bGetSyncFailCount = %d ", bGetSyncFailCount));
            if(bGetSyncFailCount % 32 == 31)
            {
                HDMIRX_DEBUG_PRINTF((" called SWReset\n"));
                SWReset_HDMIRX();
            }
            else if(bGetSyncFailCount % 8 == 7)
            {
                HDMIRX_DEBUG_PRINTF((" reset video.\n"));

                HDMIRX_WriteI2C_Byte(REG_RX_RST_CTRL, B_VDORST|B_EN_AUTOVDORST);
                delay1ms(1);
                HDMIRX_WriteI2C_Byte(REG_RX_RST_CTRL, B_EN_AUTOVDORST);
        ...
        {
            // HDMIRX_DEBUG_PRINTF(("Current Get:    ")); DumpSyncInfo(&s_CurrentVM);
            HDMIRX_DEBUG_PRINTF(("Matched Result: ")); DumpSyncInfo(&s_CurrentVM);
            bGetSyncFailCount = 0 ;
        }
        */
...
ResetAudio()
{
    ...
    //implement in all HDMIRX platforms include Splitter solution. Thanks!
    //
